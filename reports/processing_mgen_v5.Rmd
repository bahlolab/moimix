...
```
From the ECDF coverage plots we find that more than 90\% of samples are 
...
is relatively symmetric and centered at around 20, most variants have relatively
...
we apply some hard filtering to this dataset:

* select variants where at least 90% of SNPs are covered to at least 20 bases
in all samples.
* QD score > 15
* SOR < 1
* MQRankSum > -2
...
variant_filter <- cbind(gatk_df, snp_20x = snp_20x)

id_to_keep <- (variant_filter %>% filter(qd > 20, 
                                         sor < 1, 
                                         (mqrank > -2 | is.na(mqrank)), 
                                         snp_20x > 0.9))$variant.id

samples_to_keep <- field_isolates$sample[which(sample_5x > 0.9)]

seqSetFilter(mgen, sample.id = samples_to_keep, variant.id = id_to_keep)
```

This leaves us with `r length(samples_to_keep)` samples and 
`r length(id_to_keep)` variants. The samples segregate by region as follows:
...
And the variants segreagate by chromosome as follows:

```{r group-chr}
variants_df <- variant_filter %>% 
    filter(variant.id %in% id_to_keep) 

knitr::kable(variants_df %>% count(chr))
```
...
2. Esitmate the $F_{ws}$ metric to annotate samples as either single clones 
or multiple clone infections.
...
# filter gds
seqSetFilter(mgen, sample.id = centr_afr, variant.id = id_to_keep)

# estimate MAF
maf_ca <- getMAF(mgen)
# remove variants that are not polymorphic at all in this region (MAF == 0)
snp_ca <- id_to_keep[maf_ca > 0]
...
extractPED(mgen, 
           moi.estimates = moi_ca, 
           out.file = "processed_data/central_africa")
seqSetFilter(mgen)
...
maf_wa <- getMAF(mgen)
# remove variants that are not polymorphic at all in this region (MAF == 0)
snp_wa <- id_to_keep[maf_ca > 0]

seqSetFilter(mgen, sample.id = west_afr, variant.id = snp_ca)
# estimate Fws metric
fws_west_africa <- getFws(mgen)
moi_ca <- data.frame(sample = west_afr, fws = fws_central_africa, 
                     moi = ifelse(fws_central_africa > 0.95, 1, 2))
...
extractPED(mgen, 
           moi.estimates = moi_ca, 
           out.file = "processed_data/west_africa")
seqSetFilter(mgen)
```
## SE Asia
```{r se-asia}
# get central africa IDs
se_asia <- seqGetData(mgen, "sample.id")[seqGetData(mgen, "sample.annotation/region") == "SE Asia"]
# filter gds
seqSetFilter(mgen, sample.id = centr_afr, variant.id = id_to_keep)
...
maf_sea <- getMAF(mgen)
# remove variants that are not polymorphic at all in this region (MAF == 0)
snp_wa <- id_to_keep[maf_sea > 0]

seqSetFilter(mgen, sample.id = west_afr, variant.id = snp_sea)
# estimate Fws metric
fws_se_asia <- getFws(mgen)
moi_sea <- data.frame(sample = west_afr, fws = fws_se_asia, 
                     moi = ifelse(fws_central_africa > 0.95, 1, 2))
...
extractPED(mgen, 
           moi.estimates = moi_ca, 
           out.file = "processed_data/se_asia")
