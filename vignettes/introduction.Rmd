...
function. Here we filter variants based on the QD and MQ tags in the VCF.

```{r filter-quality}
# extract tags from GATK Haplotype Caller using convienence funciton
gatk_df <- getGATKInfo(isolates)

variants_to_keep <- gatk_df$variant.id[gatk_df$qd > 10 & gatk_df$mq > 50]

seqSetFilter(isolates, variant.id = variants_to_keep)
